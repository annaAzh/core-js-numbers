...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  return Math.hypot(Math.abs(x2) + Math.abs(x1), Math.abs(y2) + Math.abs(y1));
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const topRow = x1 * x2 + y1 * y2;
  const bottomX = Math.sqrt(x1 ** 2 + y1 ** 2);
  const bottomY = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(topRow / (bottomX * bottomY));
}
...
function isPowerOfTwo(num) {
  if (num === 2 ** Math.floor(Math.log2(num))) {
    return true;
  }
  return false;
}
